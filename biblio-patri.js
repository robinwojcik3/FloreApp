...
const initializeSelectionMap = (coords) => {
    ...
    const showActionPopup = (latlng) => {
        const container = L.DomUtil.create('div');
        const btnPatri = L.DomUtil.create('button', 'popup-action', container);
        btnPatri.textContent = 'Analyse patrimoniale';
        btnPatri.onclick = () => {
            map.closePopup();
            runAnalysis({ latitude: latlng.lat, longitude: latlng.lng });
        };
        const btnObs = L.DomUtil.create('button', 'popup-action', container);
        btnObs.textContent = 'Observations locales';
        btnObs.onclick = () => {
            map.closePopup();
    ...
    const startMapSelection = async () => {
        ...
        map.off('zoomstart');
        let pressTimer;
        const onSelect = (latlng) => showActionPopup(latlng);
        const onContextMenu = (e) => { e.originalEvent.preventDefault(); onSelect(e.latlng); };
        const onDown = (e) => {
            if (e.originalEvent && e.originalEvent.touches && e.originalEvent.touches.length > 1) return;
            pressTimer = setTimeout(() => onSelect(e.latlng), LONG_PRESS_MS);
        };
    ...
    const loadObservationsAt = async (params) => {
        try {
            if (!map) initializeMap(params); else map.setView([params.latitude, params.longitude], 18);
