...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const showZoningsBtn = document.getElementById('show-zonings-btn');
    ...
    const SERVICES = {
        arcgis: {
            ...
            buildUrl: (lat, lon) => {
                ...
                const b = 1000;
                return `https://www.arcgis.com/apps/webappviewer/index.html?id=bece6e542e4c42e0ba9374529c7de44c&extent=${x-b}%2C${y-b}%2C${x+b}%2C${y+b}%2C102100`;
            }
        },
        geoportail: {
        ...
        inaturalist: {
            ...
            buildUrl: (lat, lon) => {
                const r = 5;
                return `https://www.inaturalist.org/observations?lat=${lat.toFixed(8)}&lng=${lon.toFixed(8)}&radius=${r}&subview=map&threatened&iconic_taxa=Plantae`;
            }
        }
    };
    ...
    function getZoneName(props) {
        ...
        const keys = ['zone_name','nom','name','libelle','NOM','NOM_SITE','nom_zone'];
        for (const k of keys) { if (props[k]) return props[k]; if (props[k && k.toUpperCase()]) return props[k.toUpperCase()]; }
        for (const k in props) { if (typeof props[k] === 'string' && props[k]) return props[k]; }
        return 'Zonage';
    }
    ...
    function addDynamicPopup(feature, layer) {
        ...
        layer.on('click', (e) => {
            ...
            if (existing && existing.isOpen()) { if (url) window.open(url, '_blank'); }
            else {
    ...
    async function fetchAndDisplayApiLayer(name, config, lat, lon) {
        try {
            ...
            if (data && data.features && data.features.length > 0) {
                const layer = L.geoJSON(data, { renderer: L.canvas(), style: config.style, onEachFeature: addDynamicPopup });
    ...
    function displayResources(lat, lon) {
        ...
        Object.keys(SERVICES).forEach(key => {
            ...
            const url = svc.buildUrl(lat, lon);
            const card = document.createElement('div');
            card.className = 'result-card';
            card.innerHTML = `<h3>${svc.name}</h3><p>${svc.description}</p><a href="${url}" target="_blank" rel="noopener noreferrer">Ouvrir →</a>`;
            resourcesContainer.appendChild(card);
        });
    }

    async function toggleZonings() {
        if (!lastSearchCoords) return;
        if (!zoningVisible) {
            if (Object.keys(zoningLayers).length === 0) {
                for (const [name, cfg] of Object.entries(APICARTO_LAYERS)) {
                    const layer = await fetchAndDisplayApiLayer(name, cfg, lastSearchCoords.latitude, lastSearchCoords.longitude);
                    if (layer) zoningLayers[name] = layer;
                }
            }
            Object.values(zoningLayers).forEach(l => l.addTo(map));
            showZoningsBtn.textContent = 'Masquer les zonages';
            zoningVisible = true;
        } else {
            Object.values(zoningLayers).forEach(l => map.removeLayer(l));
            showZoningsBtn.textContent = 'Afficher les zonages';
            zoningVisible = false;
        }
    }
    ...
    function toggleResources() {
        ...
        if (resourcesVisible) {
            displayResources(lastSearchCoords.latitude, lastSearchCoords.longitude);
            resourcesContainer.style.display = 'grid';
            showResourcesBtn.textContent = 'Masquer les ressources';
        } else {
            resourcesContainer.style.display = 'none';
            showResourcesBtn.textContent = 'Autres ressources';
        }
    }
...
const initializeSelectionMap = (coords) => {
    ...
    const runAnalysis = async (params) => {
        try {
            ...
            lastSearchCoords = { latitude: params.latitude, longitude: params.longitude };
            if (Object.keys(zoningLayers).length) {
                Object.values(zoningLayers).forEach(l => { if (map.hasLayer(l)) map.removeLayer(l); layersControl.removeLayer(l); });
                zoningLayers = {};
            }
            if (zoningVisible) {
                for (const [name, cfg] of Object.entries(APICARTO_LAYERS)) {
                    const layer = await fetchAndDisplayApiLayer(name, cfg, params.latitude, params.longitude);
                    if (layer) { zoningLayers[name] = layer; layer.addTo(map); }
                }
            }
    ...
    if (showZoningsBtn) {
        showZoningsBtn.addEventListener('click', toggleZonings);
    }
    if (showResourcesBtn) {
        showResourcesBtn.addEventListener('click', toggleResources);
    }
