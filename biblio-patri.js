...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const showChoicePopup = (latlng, extra = {}) => {
        ...
        L.DomEvent.on(deepPatrBtn, 'click', () => {
            map.closePopup();
            showNavigation();
            runDeepAnalysis({ latitude: latlng.lat, longitude: latlng.lng, ...extra });
        });
...
let rulesByTaxonIndex = new Map();
    ...
    const fetchWithExpRetry = async (url, options = {}, maxRetries = 3) => {
        let delay = 1000;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
    ...
    const openCacheDB = () => new Promise((resolve, reject) => {
        const req = indexedDB.open('deep-gbif-cache', 1);
        req.onupgradeneeded = () => {
    ...
    const storePage = (db, page, data) => new Promise(res => {
        const tx = db.transaction('pages', 'readwrite');
        tx.objectStore('pages').put(data, page);
        tx.oncomplete = () => res();
    });

    const getAllPages = (db) => new Promise(res => {
    ...
    const clearCache = (db) => new Promise(res => {
        const tx = db.transaction('pages', 'readwrite');
...
const initializeSelectionMap = (coords) => {
    ...
    const runDeepAnalysis = async (params, blockSize = 20) => {
        try {
            lastAnalysisCoords = { latitude: params.latitude, longitude: params.longitude };
            resultsContainer.innerHTML = '';
            mapContainer.style.display = 'none';
            ...
            let resp = await fetchWithExpRetry(firstUrl);
                ...
                pages.forEach(results => {
                    results.forEach(occ => {
                        if (occ.species && occ.speciesKey) {
                            uniqueSpecies.add(occ.species);
                            if (!taxonKeyMap.has(occ.species)) taxonKeyMap.set(occ.species, occ.speciesKey);
                        }
                    });
                });
                await clearCache(db);
            ...
            for (let blockStart = 1; blockStart < totalPages; blockStart += blockSize) {
                ...
                for (let page = blockStart; page < blockEnd; page++) {
                    ...
                    resp = await fetchWithExpRetry(url);
                    if (!resp.ok) throw new Error("L'API GBIF est indisponible.");
                    const data = await resp.json();
                    await storePage(db, page, data.results || []);
                    pagesLoaded++;
                    updateProgressBar(pagesLoaded);
            ...
            const relevantRules = new Map();
            const { departement, region } = (await (await fetch(`https://geo.api.gouv.fr/communes?lat=${params.latitude}&lon=${params.longitude}&fields=departement,region`)).json())[0];
            for (const speciesName of uniqueSpecies) {
                const rulesForThisTaxon = rulesByTaxonIndex.get(speciesName);
                if (rulesForThisTaxon) {
                    for (const row of rulesForThisTaxon) {
                        let ruleApplies = false;
            ...
            for (let attempt = 1; attempt <= ANALYSIS_MAX_RETRIES; attempt++) {
                try {
                    analysisResp = await fetch('/.netlify/functions/analyze-patrimonial-status', {
                        ...
                        body: JSON.stringify({
                            relevantRules: Array.from(relevantRules.values()),
                            uniqueSpeciesNames: Array.from(uniqueSpecies),
                            coords: { latitude: params.latitude, longitude: params.longitude }
                        })
                    });
                    if (!analysisResp.ok) {
            ...
            const patrimonialMap = await analysisResp.json();
            const initialOccs = Array.from(taxonKeyMap.entries()).map(([species, speciesKey]) => ({ species, speciesKey }));
            displayResults(initialOccs, patrimonialMap, wkt);
        } catch (error) {
