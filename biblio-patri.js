...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (scrollMapBtn) {
    scrollMapBtn.addEventListener('click', () => {
      ...
      const targetTop =
        (searchControls ? searchControls.offsetTop : mapContainer.offsetTop) - offset;
  ...
  const toggleLocationTracking = (mapInstance, buttonEl) => {
    if (trackingActive) stopLocationTracking();
    else startLocationTracking(mapInstance, buttonEl);
  };
  ...
  const toggleAnalysisLabels = () => {
    ...
    if (toggleLabelsBtn) {
      toggleLabelsBtn.textContent = analysisLabelsVisible
        ? '🏷️ Masquer les étiquettes'
        : '🏷️ Afficher les étiquettes';
    }
  };
  ...
  const drawElevationProfile = () => {
    ...
    for (let x = 0; x <= totalDist; x += distSpacing) {
      const px = marginLeft + x * scaleX;
      const label =
        totalDist < 1000 ? `${Math.round(x)} m` : `${(x / 1000).toFixed(x < 1000 ? 2 : 1)} km`;
  ...
  const updateMeasureDisplay = async (latlng) => {
    ...
    for (let i = 1; i < profileSamples.length; i++) {
      ...
      if (typeof a1 === 'number' && typeof a2 === 'number') {
        const diff = a2 - a1;
        if (diff > 0) elevPos += diff;
        else elevNeg += Math.abs(diff);
      }
    }
    ...
    if (profileInfo) {
      profileInfo.innerHTML = `Distance : ${textDist}<br>D+ total : ${dPlus} m<br>D- total : ${dMinus} m`;
    }
    const elevTextParts = [];
  ...
  const floreAlpesUrl = (name) => {
    ...
    const foundKey = Object.keys(floreAlpesIndex).find(
      (key) => norm(key.split('(')[0]) === normalizedSci
    );
    if (foundKey) {
  ...
  const fetchWithRetry = async (url, options = {}, maxRetries = ANALYSIS_MAX_RETRIES) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        setStatus(
          `Erreur : ${err.message}. Nouvelle tentative (${maxRetries - attempt} restante(s))...`,
          true
        );
  ...
  const indexRulesFromCSV = (csvText) => {
    ...
    lines.forEach((line) => {
      ...
      if (rowData.nom && rowData.type) {
        if (!index.has(rowData.nom)) {
          index.set(rowData.nom, []);
        }
  ...
  const initializeMap = (params) => {
    ...
    const vegMap = L.tileLayer.wms('https://geoservices.obs-mip.fr/geoserver/vegetation/wms', {
    ...
    if (params.polygon) {
      searchAreaLayer = L.polygon(params.polygon, {
        color: '#c62828',
        weight: 2,
        fillOpacity: 0.1,
        interactive: false
      ...
      searchAreaLayer = L.circle([params.latitude, params.longitude], {
  ...
  const fetchAndDisplayAllPatrimonialOccurrences = async (
    patrimonialMap,
    wkt,
    initialOccurrences
  ) => {
    ...
    setStatus(
      `Étape 4/4: Cartographie détaillée des espèces patrimoniales... (0/${speciesNames.length})`,
      true
    );
    ...
    for (const [index, speciesName] of speciesNames.entries()) {
      setStatus(
        `Étape 4/4: Cartographie détaillée des espèces patrimoniales... (${index + 1}/${speciesNames.length})`,
        true
      );
      setProgress(75 + (index / speciesNames.length) * 25);
  ...
  const renderPatrimonialLocations = () => {
    ...
    setStatus(
      `${selectedSpecies.size} espèce(s) patrimoniale(s) cartographiée(s) sur ${pointCount} points.`,
      false
    );
    setProgress(100);
  };
  ...
  const displayResults = (occurrences, patrimonialMap, wkt) => {
    ...
    if (Object.keys(patrimonialMap).length === 0) {
      setStatus(
        `Aucune occurrence d'espèce patrimoniale trouvée dans ce rayon de ${SEARCH_RADIUS_KM} km.`
      );
      return;
    }
    ...
    setStatus(
      `${allSpeciesList.length} espèce(s) patrimoniale(s) trouvée(s). Lancement de l'étape 4/4 : cartographie détaillée...`
    );
    ...
    allSpeciesList.forEach((speciesName, index) => {
      ...
      const faLink = linkIcon(
        floreAlpesUrl(speciesName),
        'FloreAlpes.png',
        'FloreAlpes',
        'small-logo'
      );
    ...
    const updateSelectAllButton = () => {
      selectAllBtn.textContent =
        selectedSpecies.size === allPatrimonialSpecies.length
          ? 'Tout désélectionner'
          : 'Tout sélectionner';
    };

    const updateZnieffToggleButton = () => {
      toggleZnieffBtn.textContent = hideZnieffOnly
        ? 'Afficher ZNIEFF seule'
        : 'Masquer ZNIEFF seule';
    };
    ...
    table.querySelectorAll('.species-toggle').forEach((cb) => {
      cb.addEventListener('change', () => {
        ...
        if (cb.checked) selectedSpecies.add(sp);
        else selectedSpecies.delete(sp);
  ...
  const runAnalysis = async (params, excludeZnieff = false) => {
    ...
    try {
      ...
      let wkt = params.wkt;
      if (!wkt) {
        wkt = `POLYGON((${Array.from({ length: 33 }, (_, i) => {
      ...
      setStatus(
        `Étape 2/4: Inventaire de la flore locale via GBIF... (Page 0/${pagesToFetch})`,
        true
      );
      for (let page = 0; page < pagesToFetch; page++) {
        setProgress(25 + (page / pagesToFetch) * 25);
        const offset = page * limit;
        setStatus(
          `Étape 2/4: Inventaire de la flore locale via GBIF... (Page ${page + 1}/${pagesToFetch})`,
          true
        );
      ...
      if (typeof showNotification === 'function' && totalPages) {
        if (retrievedPages < totalPages) {
          showNotification(
            `Résultats partiels : ${retrievedPages} pages récupérées sur ${totalPages} disponibles`,
            'warning'
          );
        } else {
          showNotification(
            `Résultats complets : ${retrievedPages} pages récupérées sur ${totalPages}`
          );
        }
      }
      ...
      const { departement, region } = (
        await (
          await fetch(
            `https://geo.api.gouv.fr/communes?lat=${params.latitude}&lon=${params.longitude}&fields=departement,region`
          )
        ).json()
      )[0];
      for (const speciesName of uniqueSpeciesNames) {
        ...
        if (rulesForThisTaxon) {
          for (const row of rulesForThisTaxon) {
            ...
            const isHabitatsDirective =
              type.includes('directive habitat') && HABITATS_DIRECTIVE_CODES.has(row.code);
            if (isHabitatsDirective) {
              ...
              if (adminCode === departement.code || adminCode === region.code) {
                ruleApplies = true;
              }
            }
            if (ruleApplies) {
              ...
              if (isRedList && nonPatrimonialRedlistCodes.has(row.code)) {
                continue;
              }
              const ruleKey = `${row.nom}|${row.type}|${row.adm}`;
              if (!relevantRules.has(ruleKey)) {
                const descriptiveStatus = isRedList
                  ? `${row.type} (${row.code}) (${row.adm})`
                  : row.label;
      ...
      for (let attempt = 1; attempt <= ANALYSIS_MAX_RETRIES; attempt++) {
        try {
          ...
          setStatus(
            `Erreur : ${err.message}. Nouvelle tentative (${ANALYSIS_MAX_RETRIES - attempt} restante(s))...`,
            true
          );
          await new Promise((res) => setTimeout(res, RETRY_DELAY_MS));
        }
  ...
  const handleGeolocationSearch = async (showPopup = true) => {
    try {
      ...
      setStatus(`Erreur de géolocalisation : ${error.message}`);
    }
  };
  ...
  const updatePolygonPreview = () => {
    ...
    if (polygonPoints.length >= 3) {
      ...
      polygonPreview = L.circleMarker(polygonPoints[0], {
        radius: 4,
        color: '#c62828',
        fillColor: '#c62828',
        fillOpacity: 1,
  ...
  const displayObservations = (occurrences) => {
    ...
    const floraOccs = occurrences.filter(
      (o) =>
        (o.phylum && /tracheophyta/i.test(o.phylum)) || (o.kingdom && /plantae/i.test(o.kingdom))
    );
    floraOccs.forEach((o) => {
  ...
  const loadObservationsAt = async (params) => {
    try {
      ...
      if (obsSearchCircle) {
        map.removeLayer(obsSearchCircle);
        obsSearchCircle = null;
      }
      if (obsSearchPolygon) {
        map.removeLayer(obsSearchPolygon);
        obsSearchPolygon = null;
      }
      let wkt;
      if (params.wkt) {
        obsSearchPolygon = L.polygon(params.polygon, {
          color: '#c62828',
          weight: 2,
          fillOpacity: 0.1,
          interactive: false
        ...
        obsSearchCircle = L.circle([params.latitude, params.longitude], {
          radius: OBS_RADIUS_KM * 1000,
          color: '#c62828',
          weight: 2,
          fillOpacity: 0.1,
          interactive: false
