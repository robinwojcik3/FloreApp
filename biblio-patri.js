...
const initializeSelectionMap = (coords) => {
    ...
    const SERVICES = {
        arcgis: {
            ...
            buildUrl: (lat, lon) => {
                ...
                const buffer = 3000; // larger extent for a wider view
        ...
        wetlands: {
            ...
            description: "Consulter les zones humides potentielles",
            color: "#00695c",
            icon: 'data:image/svg+xml;base64,',
            buildUrl: (lat, lon) => {
                if (typeof proj4 === 'undefined') {
                    return 'https://sig.reseau-zones-humides.org/';
                }
    ...
    const openResourcesPopup = async (latlng) => {
        ...
        if (typeof altitude === 'number') {
            const stage = getAltitudinalStage(altitude);
            const info = L.DomUtil.create('p', 'altitude-info', container);
            info.textContent = `Altitude : ${Math.round(altitude)} m${stage ? ` (\u00E9tage ${stage})` : ''}`;
        }
        Object.values(SERVICES).forEach(s => {
            const url = s.buildUrl(latlng.lat, latlng.lng);
