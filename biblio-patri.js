...
const initializeSelectionMap = (coords) => {
    ...
    const runDeepAnalysis = async (params) => {
        try {
            lastAnalysisCoords = { latitude: params.latitude, longitude: params.longitude };
            resultsContainer.innerHTML = '';
            mapContainer.style.display = 'none';
            ...
            setStatus('Étape 2/4: Extraction approfondie via GBIF...', true);
            const resp = await fetchWithRetry(`/.netlify/functions/gbif-deep-search?geometry=${encodeURIComponent(wkt)}`);
