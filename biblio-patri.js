...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const initializeMap = (params, radiusKm = SEARCH_RADIUS_KM) => {
        stopLocationTracking();
        ...
        if (params.polygon) {
            ...
            searchAreaLayer = L.circle([params.latitude, params.longitude], { radius: radiusKm * 1000, color: '#c62828', weight: 2, fillOpacity: 0.1, interactive: false }).addTo(map);
        }
    };
...
const initializeSelectionMap = (coords) => {
    ...
    const handleAddressSearch = async () => {
        ...
        try {
            ...
            setStatus('Déplacez la carte ou sélectionnez un point pour lancer une analyse.', false);
    ...
    const startMapSelection = async () => {
        ...
        const selectPoint = (latlng) => {
            const actions = [
                { label: 'Flore patrimoniale', onSelect: () => { cleanup(); runAnalysis({ latitude: latlng.lat, longitude: latlng.lng }); } },
                { label: 'Toutes les espèces', onSelect: () => { cleanup(); loadObservationsAt({ latitude: latlng.lat, longitude: latlng.lng }); } },
                { label: 'Annuler', onSelect: () => {} }
            ];
            if (typeof showChoiceModal === 'function') {
                showChoiceModal('Que souhaitez-vous faire ?', actions);
            } else if (confirm("Lancer l'analyse patrimoniale ? (Annuler pour la liste des espèces)")) {
                cleanup();
