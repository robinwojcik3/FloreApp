...
async function loadPdfViewer() {
  ...
  if (!pdfUrl) {
    viewerContainer.innerHTML =
      '<div class="error-message"><h1>Erreur : Aucun fichier PDF spécifié.</h1></div>';
    return;
  }
  ...
  try {
    ...
    if (ocrBtn) {
      ocrBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          const name = genus ? `${capitalizeGenus(genus)}.txt` : 'extrait.txt';
    ...
    displayFallback(
      ...
      'Impossible de charger le lecteur PDF.',
      pdfUrl,
      initialPageNum
    );
  }
